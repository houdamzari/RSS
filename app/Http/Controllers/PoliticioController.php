...
use App\Models\PoliticioModel;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;
use Vedmant\FeedReader\Facades\FeedReader;

class PoliticioController extends Controller
{
    public function TrendNews()
    {
        ...
        $data = PoliticioModel::latest()->get();

        $items = $f->get_items();
        foreach ($items as $item) {
            $media = PoliticioModel::firstOrNew(['title' => $item->get_title()]);
        ...
        return view('HomePress.trendnews')->with('media', $data);
    }
}
